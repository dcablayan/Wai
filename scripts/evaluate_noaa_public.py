...
Offline / CI use
----------------
Set NOAA_OFFLINE=1 in the environment to skip network calls and use the
bundled mock fixtures instead (for CI without internet access).

Usage
-----
    python -m scripts.evaluate_noaa_public               # live NOAA fetch
    NOAA_OFFLINE=1 python -m scripts.evaluate_noaa_public  # offline mode
"""
...
def fetch_noaa_df(
    ...
    if offline:
        return _make_mock_noaa_df(station_id, begin, end, lat, lon, seed=hash(station_id) % 2**31)
    ...
    try:
        ...
        warnings.warn(
            ...
            "Using mock fixture instead.",
            stacklevel=2,
        )
        return _make_mock_noaa_df(station_id, begin, end, lat, lon)
...
def evaluate_station(
    ...
    n = len(df)
    if n < 100:
        return {"error": f"Insufficient data (n={n})", "station": station_label}
    ...
    try:
        ...
        harm_metrics["mae_ci_95"] = bootstrap_ci(y_te.values, h_preds, n_boot=500)
    except Exception as exc:
    ...
    try:
        ...
        gb_metrics["mae_ci_95"] = bootstrap_ci(y_te.values, gb_preds, n_boot=500)
    except Exception as exc:
...
def format_results_md(results: dict) -> str:
    ...
    for station_id, res in results.items():
        ...
        lines.append(f"## {label} — {holdout} holdout")
...
def main() -> None:
    offline = os.environ.get("NOAA_OFFLINE", "").strip() in ("1", "true", "yes")
    if offline:
        print("NOAA_OFFLINE=1 — using mock fixtures (no network calls).")
    else:
        print("Fetching real NOAA CO-OPS observations …")

    all_results: dict = {}

    for station_id, label, lat, lon in NOAA_STATIONS:
        print(f"\n  → {label} ({station_id}) …")
        df = fetch_noaa_df(station_id, EVAL_BEGIN, EVAL_END, lat, lon, offline=offline)
        res = evaluate_station(df, station_label=label, holdout_type="temporal")
        ...
        if "mae" in m_h:
            ...
            print(f"    HarmonicRidge  MAE={m_h['mae']:.4f}  "
                  f"RMSE={m_h['rmse']:.4f}  R²={m_h['r2']:.4f}{ci_str}")
        elif "error" in m_h:
    ...
    storm_df = fetch_noaa_df(STORM_STATION_ID, STORM_BEGIN, STORM_END,
                             storm_lat, storm_lon, offline=offline)
    storm_res = evaluate_station(storm_df, station_label=f"{storm_label} (storm period)",
                                 holdout_type="event")
