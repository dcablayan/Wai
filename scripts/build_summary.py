...
The summary records a source-content fingerprint snapshot.  Because any
static JSON file can become stale after it is written, consumers must run
``python -m scripts.check_report_freshness`` for a current verdict rather
than trusting a persisted boolean.
"""
...
def _staleness(run_meta: Optional[dict]) -> dict:
    snapshot = evidence_freshness(run_meta, REPO_ROOT)
    return {
        "verification_method": snapshot["verification_method"],
        "recorded_source_fingerprint": snapshot["recorded_source_fingerprint"],
        "current_source_fingerprint_at_generation": snapshot[
            "current_source_fingerprint"
        ],
        "fresh_at_generation": snapshot["fresh_at_verification"],
        "fresh": None,
        "note": (
            "No static artifact can assert current freshness. Run "
            "`python -m scripts.check_report_freshness` for a live verdict."
        ),
    }
...
def main() -> None:
    ...
    if not stl["fresh_at_generation"]:
        print(
            "\nWARNING: generated reports do not match current source inputs.\n"
            f"  current={stl['current_source_fingerprint_at_generation']}\n"
            f"  recorded={stl['recorded_source_fingerprint']}\n"
            "  Re-run `make demo` so the artifacts match the current code."
        )
